...
const Ventas = () => {
  ...
  const submitForm = async (e) => {
    ...
    else { window.alert("No hay Registros de VENTA"); }
    ;
  };

  return (    
    <div className='flex h-full w-full items-center justify-center'> 
      <form ref={form} onSubmit={submitForm} className='flex flex-col h-full w-full mt-8 px-5 py-2'>
        <h1 class="text-6xl font-bold tracking-tight text-gray-700">Crear una nueva venta</h1>  
        <label className='flex flex-col mt-8' htmlFor='vendedor'>
          <span className='text-2xl font-gray-900 font-bold'>Vendedor:</span>
          <select name='vendedor' className='p-2 border-2 border-gray-700' defaultValue='' required>
          ...
          <span className='text-2xl font-gray-900 mt-6 font-bold'>Cliente:</span>
          <select name='cliente' className='p-2 border-2 border-gray-700' defaultValue='' required>
        ...
        <span className='text-2xl font-gray-900 mt-6 font-bold'>Productos:</span>
        <TablaVehiculos
          productos={productos}
...
const TablaVehiculos = ({ productos, setVehiculos, setVehiculosTabla }) => {
  ...
  */
  useEffect(() => {
    ...
    var table = document.getElementById('tabla'), rows = table.getElementsByTagName('tr'), i, j, cells, customerId;
    var total = 0;
  ...
  return (
    <div>
      <div className='flex'>
        <label className='flex flex-col border-2 border-gray-700' htmlFor='vehiculo'>
          <select
            className='p-2'
            name='producto'
            id='producto'
...
const FilaVehiculo = ({ veh, index, eliminarVehiculo, modificarVehiculo }) => {
  ...
  return (
    <tr name={`fila_${index}`}>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setVehiculo({
                ...
                total:
                  parseFloat(vehiculo.valor) * parseFloat(e.target.value === '' ? '0' : e.target.value),
      ...
      <td>{vehiculo.cantidad > 0 ? parseFloat(vehiculo.costo * vehiculo.cantidad) : '0'}</td>
      <td>
        <i
          onClick={(e) => {
